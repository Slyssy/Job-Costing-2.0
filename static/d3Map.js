...
// Load in Cities Data
d3.tsv("static/uscities.tsv").then((data) => {
  // console.log(data)
  cityG
    .selectAll("circle")
    .raise()
...
const mapData = projectArray.map(
  ({
    project_name,
    project_address,
    act_start_date,
    ...
    fin_act_gross_profit: parseFloat(fin_act_gross_profit.replace(/,/g, "")),
    lat: lat.toString(),
    lng: lng.toString(),
  })
);
...
const mapData1 = mapData0.map((o) => ({ ...o, month: month[o.month - 1] }));
...
const selectYears = mapYears.filter(
