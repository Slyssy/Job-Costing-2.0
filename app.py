...
def password_update_db():
    ...
    return render_template('updatePassword copy.html')
    ...
    if request.method == 'POST':
        ...
        input_log_in = request.form['log_in']
        
        # grab info from db
        cur = conn.cursor()
        cur.execute('SELECT log_in FROM users;')
        log_in_fetch = cur.fetchall()
        print("fetched info")
        print(log_in_fetch)
        print("-----------------------------")
        ...
        for db_row in log_in_fetch:
            log_in_names_dict = {}
            db_row = functools.reduce(operator.add,(db_row))
            # log_in_names_dict['login_name'] = db_row
            log_in_names_list.append(db_row)
        
        if input_log_in in log_in_names_list:
            cur = conn.cursor()
            cur.execute('SELECT * FROM users WHERE log_in=%s;',[input_log_in])
            rows = cur.fetchall()
            print("this is the fetch based off username input")
            print(rows)
            print("-----------------------------")
            ...
            print(error)
            return render_template('updatePassword copy.html', error=error)
        



        #if no match error message
        # if matches - check answers to match 
        # if answers match, update password in db
