...
try:
    conn = psycopg2.connect(
        dbname=pg_dbname, host=pg_ipaddress, user=pg_username, password=pg_password
    )
    print("------------------------------------")
...
def dashboard_data():
    if request.method == "GET":
        ...
        for proj in project_details_data:
            ...
            cur = conn.cursor()
            cur.execute(
                "SELECT * FROM expenses WHERE project_id=" + str(project_id) + ";"
            )
            ...
            project_dict["fin_act_miscellaneous_expense"] = "{:,.2f}".format(
                float(fin_act_misc_exp)
            )
            project_dict["fin_act_subcontractor_expense"] = "{:,.2f}".format(
                float(fin_act_sub_exp)
            )
            project_dict["fin_act_overhead_expense"] = "{:,.2f}".format(
                float(est_overhead_expense)
            )

            # Fetch Time_Sheets data for given project_id
            cur = conn.cursor()
            cur.execute(
                "SELECT * FROM time_sheets WHERE project_id=" + str(project_id) + ";"
            )
            ...
            if not len(timesheet_data):
                print(
                    "No timesheets entered in database for this project, therefore skipping Project ID "
                    + str(project_id)
                )
            for timesheet in timesheet_data:
                # Calling the pre-defined function
                (timesheet_dict, act_labor_hours) = get_timesheet_dict(
                    timesheet, act_labor_hours, conn
                )
            ...
            project_dict["fin_est_revenue"] = "{:,.2f}".format(revenue)
            ...
            fin_est_labor_expense = float(fin_est_labor_hours) * float(
                fin_est_labor_rate
            )
            project_dict["fin_est_labor_expense"] = "{:,.2f}".format(
                fin_est_labor_expense
            )
            fin_est_material_expense = float(est_material_expense)
            project_dict["fin_est_material_expense"] = "{:,.2f}".format(
                fin_est_material_expense
            )
            fin_est_subcontractor_expense = float(est_subcontractor_expense)
            project_dict["fin_est_subcontractor_expense"] = "{:,.2f}".format(
                fin_est_subcontractor_expense
            )
            fin_est_miscellaneous_expense = float(est_miscellaneous_expense)
            project_dict["fin_est_miscellaneous_expense"] = "{:,.2f}".format(
                fin_est_miscellaneous_expense
            )
            fin_est_overhead_expense = float(est_overhead_expense)
            project_dict["fin_est_overhead_expense"] = "{:,.2f}".format(
                fin_est_overhead_expense
            )
            fin_est_gross_profit = (
                float(fin_est_revenue)
                - float(fin_est_labor_expense)
                - float(fin_est_material_expense)
                - float(fin_est_subcontractor_expense)
                - float(fin_est_miscellaneous_expense)
                - float(fin_est_overhead_expense)
            )
            # project_dict['fin_est_gross_profit'] = "{:.2f}".format(fin_est_gross_profit)
            project_dict["fin_est_gross_profit"] = "{:,.2f}".format(
                fin_est_gross_profit
            )
            fin_est_gross_margin = (
                float(fin_est_gross_profit) / float(fin_est_revenue) * 100
            )
            project_dict["fin_est_gross_margin"] = (
                "{:,.2f}".format(fin_est_gross_margin) + " %"
            )
            ...
            fin_act_labor_expense = float(fin_act_labor_hours) * float(
                fin_act_labor_rate
            )
            project_dict["fin_act_labor_expense"] = "{:,.2f}".format(
            ...
            fin_act_gross_margin = (
                float(fin_act_gross_profit) / float(fin_act_revenue) * 100
            )
            project_dict["fin_act_gross_margin"] = (
                "{:.2f}".format(fin_act_gross_margin) + " %"
            )
        pprint(project_list)
...
def new_project_data():
    ...
    if request.method == "POST":
        ...
        est_labor_expense = str(
            "{:.2f}".format(float(est_labor_hours) * float(est_labor_rate))
        )
        full_values_string += "," + est_labor_expense
        # adding in new expenses to grab from user input
        est_material_expense = str(
            "{:.2f}".format(float(request.form["est_material_expense"]))
        )
        full_values_string += "," + est_material_expense
        est_subcontractor_expense = str(
            "{:.2f}".format(float(request.form["est_subcontractor_expense"]))
        )
        full_values_string += "," + est_subcontractor_expense
        est_miscellaneous_expense = str(
            "{:.2f}".format(float(request.form["est_miscellaneous_expense"]))
        )
        ...
        if "act_start_date" in request.form and request.form["act_start_date"] != "":
            act_start_date = datetime.datetime.strptime(
                request.form["act_start_date"], "%m/%d/%Y"
            ).date()
        else:
        ...
        try:
            ...
            cur.execute(
                "INSERT INTO project_details (name, street, street2, city, state, zip, revenue, est_labor_rate, est_labor_hours, est_labor_expense,  est_material_expense, est_subcontractor_expense, est_miscellaneous_expense, est_overhead_expense, act_start_date) VALUES "
                + full_values_string
                + ";"
            )
...
def userdata_html_to_db():
    ...
    if request.method == "POST":
        ...
        try:
            cur.execute(
                "INSERT INTO users (name, job_title, pay_rate, email, phone, log_in, password) VALUES "
                + full_values_string
                + ";"
            )
            print("-----------------------------------")
...
def time_html_to_db():
    if request.method == "GET":
        ...
        return render_template(
            "enterTime.html", dropdown_dict=json.dumps(dropdown_dict)
        )
    ...
    if request.method == "POST":
        ...
        if len(missing_fields):
            missing_fields_error = "Oops - could not find these fields " + " ".join(
                missing_fields
            )
        ...
        for user in user_id_fetch:
            user_id = user[0]
        cur.execute(
            "SELECT project_id FROM project_details WHERE name=%s;", [project_name]
        )
        ...
        start_time = datetime.datetime.strptime(start_time, "%m/%d/%Y %H:%M").strftime(
            "%Y-%m-%d %H:%M:%S"
        )
        ...
        try:
            cur = conn.cursor()
            cur.execute(
                "INSERT INTO time_sheets (user_id, project_id, start_time, finish_time) VALUES (%s, %s, %s, %s)",
                (user_id, project_id, start_time, finish_time),
            )
            print("-----------------------------------")
            print("Data added to database - woohoo!")
            print("-----------------------------------")
...
def project_expense():
    if request.method == "GET":
        ...
        return render_template(
            "enter_expense.html", dropdown_dict=json.dumps(dropdown_dict)
        )
    # if request.method == 'GET':
    #     print('*****************')
    #     print('Getting form...')
    ...
    if request.method == "POST":
        ...
        try:
            cur.execute(
                "INSERT INTO expenses (expense_type, expense_amount, project_id) VALUES "
                + full_values_string
                + ";"
            )
            print("-----------------------------------")
...
def project_search():
    ...
    if request.method == "POST":
        ...
        try:
            sql_insert_string = (
                "UPDATE project_details SET act_comp_date = TO_DATE('"
                + act_end_date
                + "', 'MM/DD/YYYY') WHERE project_id="
                + project_id
                + ";"
            )
...
def user_data_update_db():
    if request.method == "GET":
        ...
        pprint(dropdown_dict)
        return render_template(
            "updateUser.html", dropdown_dict=json.dumps(dropdown_dict)
        )
    ...
    if request.method == "POST":
        ...
        cur = conn.cursor()
        print(
            f"UPDATE users SET name ='{name}', job_title ='{job_title}' , pay_rate ='{pay_rate}', email ='{email}', phone = '{phone}', log_in ='{log_in}', password = '{passw}'  WHERE user_id = {db_user_id}"
        )
        # Adding form input data to PostgreSQL database
        try:
            cur.execute(
