...
def search_by_id(project_id, conn):
    ...
    for proj in project_details_data:
        ...
        project_list["fin_est_overhead_expense"] = "{:.2f}".format(
        ...
        project_list["fin_est_gross_margin"] = (
        ...
        project_list["fin_act_subcontractor_expense"] = "{:.2f}".format(
            fin_act_subcon_exp
        )
        fin_act_misc_exp = float(total_miscellaneous_exp)
        project_list["fin_act_miscellaneous_expense"] = "{:.2f}".format(
            fin_act_misc_exp
        )
        # if we decide to keep oh expense as an amoutn entry (not %) in db
        fin_act_overhead_expense = float(est_overhead_expense)
        project_list["fin_act_overhead_expense"] = "{:.2f}".format(
            fin_act_overhead_expense
        )
        ...
        fin_act_gross_profit = round(
            (
                float(fin_act_revenue)
                - float(fin_act_labor_expense)
                - fin_act_mat_exp
                - fin_act_subcon_exp
                - fin_act_misc_exp
                - fin_act_overhead_expense
        ...
        project_list["fin_act_gross_margin"] = (
...
def get_timesheet_dict(timesheet, act_labor_hours, conn):
    ...
    cur = conn.cursor()
    cur.execute(
        "SELECT user_id, name, pay_rate FROM users WHERE user_id=" + str(user_id)
    )
...
def get_actual_labor_rate(timesheet_all, act_labor_hours, conn):
    ...
    for timesheet_dict in timesheet_all:
        sum_of_hours_t_rate += float(timesheet_dict["hours_worked"]) * float(
            timesheet_dict["hourly_pay_rate"]
        )
